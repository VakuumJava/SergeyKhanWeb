          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {masterData.length > 0 
                  ? Math.round(
                      masterData.reduce((sum, master) => {
                        // Считаем процент загрузки как количество заказов сегодня
                        // Максимальное количество заказов в день - 10 (можно настроить)
                        const maxOrdersPerDay = 10;
                        const percentage = (master.total_orders_today / maxOrdersPerDay) * 100;
                        return sum + Math.min(percentage, 100); // ограничиваем 100%
                      }, 0) / masterData.length
                    )
                  : 0
                }%
