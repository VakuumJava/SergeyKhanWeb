...
from .models import Balance, CustomUser, DistanceSettingsModel, Order
...
def calculate_average_check(master_id, orders_count=10):
    ...
    recent_orders = Order.objects.filter(
        assigned_master=master, status="завершен", final_cost__isnull=False
...
def check_distance_level(master_id):
    ...
    if (
        daily_revenue >= settings.daily_order_sum_threshold
        or net_turnover >= settings.net_turnover_threshold
    ):
        return 2
...
def get_visible_orders_for_master(master_id):
    ...
    # Возвращаем заказы, которые видны мастеру
    ...
    return Response(
        {
            ...
            "meets_requirements": {
                ...
                "daily_distance_by_revenue": daily_revenue
                >= settings.daily_order_sum_threshold,
                "daily_distance_by_turnover": net_turnover
                >= settings.net_turnover_threshold,
            },
        }
    )
...
def get_master_available_orders_with_distance(request):
    ...
    # Используем публичный сериализатор для мастеров, которые еще не взяли
    # заказ
    ...
    return Response({"orders": serializer.data, "distance_info": distance_info})
...
def set_master_distance_manually(request, master_id):
    ...
    if distance_level not in [0, 1, 2]:
        return Response(
            {"error": "Invalid distance level. Must be 0, 1, or 2"},
        ...
        orders_data.append(
            {
                ...
                "status": order.status,
                "estimated_cost": (
                    str(order.estimated_cost) if order.estimated_cost else None
                ),
                "created_at": (
    ...
    return Response(
        {
            "distance_info": {
                ...
                "meets_requirements": {
                    ...
                    "level_2": (
                        float(daily_revenue) >= settings.daily_order_sum_threshold
                        or float(net_turnover) >= settings.net_turnover_threshold
                    ),
                },
            },
            "orders": orders_data,
...
def get_master_distance_status(request):
    ...
    if request.user.role != "master":
        return Response(
            {"error": "Access denied. Only masters can access this endpoint."},
            status=403,
        )
