...
from .models import (Balance, BalanceLog, CalendarEvent, CompanyBalance,
                     ...
                     FinancialTransaction, MasterAvailability,
                     MasterProfitSettings, Order, OrderCompletion, OrderLog,
                     ProfitDistributionSettings, SystemLog, TransactionLog)
                    ...
                    if request:
                        ...
                        for photo_path in obj.completion.completion_photos:
                            if photo_path.startswith("completion_photos/"):
                                ...
                                full_url = request.build_absolute_uri(
                                    f"/media/completion_photos/{photo_path}"
                                )
                            photo_urls.append(full_url)
                        completion_data["completion_photos"] = photo_urls
                    else:
                        completion_data["completion_photos"] = (
                            obj.completion.completion_photos
                        )
...
class OrderDetailSerializer(serializers.ModelSerializer):
    ...
    full_address = serializers.CharField(source="get_full_address", read_only=True)
    public_address = serializers.CharField(source="get_public_address", read_only=True)
    assigned_master_email = serializers.CharField(
        source="assigned_master.email", read_only=True
    )
    ...
    transferred_to_email = serializers.CharField(
        source="transferred_to.email", read_only=True
    )
    completion = serializers.SerializerMethodField()
...
class BalanceLogSerializer(serializers.ModelSerializer):
    ...
    balance_type_display = serializers.CharField(
        source="get_balance_type_display", read_only=True
    )
...
class OrderCompletionCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_order(self, value):
        ...
        if value.status not in ["выполняется", "назначен"]:
            raise serializers.ValidationError(
                "Заказ должен быть в статусе 'выполняется' или 'назначен'"
            )
    ...
    def create(self, validated_data):
        ...
        completion_photos = (
            request.FILES.getlist("completion_photos") if request else []
        )
        ...
        if completion_photos:
            ...
            try:
                ...
                completion_photos_dir = os.path.join(
                    settings.MEDIA_ROOT, "completion_photos"
                )
                ...
                for i, photo in enumerate(completion_photos):
                    ...
                    if not photo.content_type.startswith("image/"):
                        print(
                            f"Файл {
                                photo.name} не является изображением: {
                                photo.content_type}"
                        )
                        continue
                    ...
                    filename = f"completion_{
                        completion.id}_{timestamp}_{unique_id}_{i}{file_extension}"
...
class FinancialTransactionSerializer(serializers.ModelSerializer):
    ...
    order_id = serializers.IntegerField(
        source="order_completion.order.id", read_only=True
    )
