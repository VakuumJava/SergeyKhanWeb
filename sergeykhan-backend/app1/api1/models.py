...
class Balance(models.Model):
    ...
    paid_amount = models.DecimalField(
        max_digits=12, decimal_places=2, default=0.00
    )  # Выплаченная сумма за все время
...
class Order(models.Model):
    ...
    street = models.CharField(
        max_length=255, null=True, blank=True, verbose_name="Улица"
    )
    house_number = models.CharField(
        max_length=50, null=True, blank=True, verbose_name="Номер дома"
    )
    apartment = models.CharField(
        max_length=50, null=True, blank=True, verbose_name="Квартира"
    )
    entrance = models.CharField(
        max_length=50, null=True, blank=True, verbose_name="Подъезд"
    )
    ...
    # Поле для указания тестового заказа
    ...
    service_type = models.CharField(
        max_length=100, null=True, blank=True, verbose_name="Тип услуги"
    )
    equipment_type = models.CharField(
        max_length=100, null=True, blank=True, verbose_name="Тип оборудования"
    )
    promotion = models.CharField(
        max_length=255, null=True, blank=True, verbose_name="Акции"
    )
...
class BalanceLog(models.Model):
    ...
    balance_type = models.CharField(
        max_length=10, choices=BALANCE_TYPE_CHOICES, default="current"
    )
    action_type = models.CharField(
        max_length=10, choices=ACTION_TYPE_CHOICES, default="top_up"
    )
    ...
    action = models.CharField(max_length=100, default="legacy")
    created_at = models.DateTimeField(auto_now_add=True)
...
class CalendarEvent(models.Model):
    master = models.ForeignKey(
        CustomUser, on_delete=models.CASCADE, related_name="calendar_events"
    )
...
class Contact(models.Model):
    ...
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default="не обзвонен"
    )
...
class DistanceSettingsModel(models.Model):
    ...
    visible_period_standard = models.PositiveIntegerField(
        default=28, help_text="Количество часов видимости для обычной дистанционки"
    )
    ...
    visible_period_daily = models.PositiveIntegerField(
        default=48, help_text="Количество часов видимости для суточной дистанционки"
    )
...
class OrderCompletion(models.Model):
    ...
    order = models.OneToOneField(
        Order, on_delete=models.CASCADE, related_name="completion"
    )
    master = models.ForeignKey(
        CustomUser,
        on_delete=models.SET_NULL,
    ...
    completion_photos = models.JSONField(
        default=list, blank=True, verbose_name="Фотографии выполненных работ"
    )

    # Финансовые данные
    parts_expenses = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        default=0,
        verbose_name="Расходы на запчасти (₸)",
    )
    ...
    total_received = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        verbose_name="Полная сумма получена за заказ (₸)",
    )

    # Автоматически рассчитываемые поля
    total_expenses = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, verbose_name="Общие расходы (₸)"
    )
    net_profit = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, verbose_name="Чистая прибыль (₸)"
    )
    ...
    curator_notes = models.TextField(
        blank=True, null=True, verbose_name="Заметки куратора"
    )

    # Распределение средств
    is_distributed = models.BooleanField(
        default=False, verbose_name="Средства распределены"
    )
    ...
    def calculate_distribution(self):
        ...
        from .models import \
            MasterProfitSettings  # Избегаем циклического импорта

        settings = MasterProfitSettings.get_settings_for_master(master)

        # Используем новые поля для распределения
        master_immediate = self.net_profit * (
            Decimal(settings["master_paid_percent"]) / 100
        )
        master_deferred = self.net_profit * (
            Decimal(settings["master_balance_percent"]) / 100
        )
...
class FinancialTransaction(models.Model):
    ...
    order_completion = models.ForeignKey(
        OrderCompletion,
        on_delete=models.CASCADE,
