...
from .models import CustomUser, MasterAvailability, Order
...
def analyze_day_capacity(target_date, masters):
    ...
    masters_with_orders = masters.filter(orders__scheduled_date=target_date).distinct()

    # Подсчет слотов времени
    total_time_slots = MasterAvailability.objects.filter(date=target_date).count()

    # Занятые слоты (заказы на этот день)
    occupied_slots = Order.objects.filter(
        scheduled_date=target_date, status__in=["назначен", "выполняется", "в работе"]
    ).count()
    ...
    # Максимальное количество заказов, которое может выполнить один мастер за
    # день
    ...
    theoretical_max_capacity = (
        len(masters_with_availability) * max_orders_per_master_per_day
    )
    ...
    masters_without_schedule = masters.exclude(availability_slots__date=target_date)
        ...
        "masters_details": [
            {
                ...
                "assigned_orders": Order.objects.filter(
                    assigned_master=master, scheduled_date=target_date
                ).count(),
        ...
        week_forecast.append(
            {
                ...
                "masters_available": day_analysis["masters_stats"][
                    "masters_with_availability"
                ],
                "utilization_percent": day_analysis["capacity"][
    ...
    return Response(
        {
            ...
            "total_week_capacity": sum(
                day["available_capacity"] for day in week_forecast
            ),
