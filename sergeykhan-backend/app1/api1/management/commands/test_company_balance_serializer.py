...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS("🔧 Тестирование CompanyBalanceLogSerializer")
        )
        ...
        try:
            ...
            admin_user = User.objects.create_user(
                email="test@admin.com", password="test123", role="super_admin"
            )
        ...
        company_balance, created = CompanyBalance.objects.get_or_create(
            id=1, defaults={"amount": 50000.00}
        )
        if created:
            self.stdout.write(
                f"✅ Создан баланс компании: {
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f"❌ Ошибка множественной сериализации: {e}")
            )
            return
        ...
        if serialized_data:
            ...
            if missing_fields:
                self.stdout.write(
                    self.style.WARNING(f"⚠️  Отсутствующие поля: {missing_fields}")
                )

            if extra_fields:
                self.stdout.write(
                    self.style.WARNING(f"⚠️  Дополнительные поля: {extra_fields}")
                )

            if not missing_fields and not extra_fields:
                self.stdout.write(
                    self.style.SUCCESS("✅ Все ожидаемые поля присутствуют!")
                )

        self.stdout.write("=" * 60)
        self.stdout.write(
            self.style.SUCCESS("🎉 Тестирование сериализатора завершено успешно!")
        )
