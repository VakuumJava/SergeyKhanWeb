...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS("🔧 Полный тест API endpoints баланса компании")
        )
        ...
        try:
            ...
            admin_user = User.objects.create_user(
                email="testadmin@example.com", password="admin123", role="super-admin"
            )
        ...
        try:
            response = requests.get(
                f"{base_url}/api/company-balance/logs/", headers=headers
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                "✅ Internal Server Error исправлен - API работает корректно!"
            )
        )
