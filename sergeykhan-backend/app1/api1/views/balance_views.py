...
def get_user_balance(request, user_id):
    balance_obj, _ = Balance.objects.get_or_create(
        user_id=user_id, defaults={"amount": 0}
    )
...
def top_up_balance(request, user_id):
    ...
    log_transaction(
        ...
        amount=amt,
        description=f"Пополнение баланса пользователя {
            user.email} на сумму {amt}. Баланс: {old_balance} → {
            balance.amount}",
...
def deduct_balance(request, user_id):
    ...
    log_transaction(
        ...
        amount=amt,
        description=f"Списание с баланса пользователя {
            user.email} на сумму {amt}. Баланс: {old_balance} → {
            balance.amount}",
        performed_by=request.user,
    )
...
def get_financial_transactions(request):
    ...
    transactions = FinancialTransaction.objects.filter(user=request.user).order_by(
        "-created_at"
    )
...
def fine_master(request):
    ...
    balance, _ = Balance.objects.get_or_create(
        user=master, defaults={"amount": Decimal("0.00")}
    )
    ...
    log_transaction(
        ...
        transaction_type="balance_deduct",
        amount=amount,
        description=f"Штраф от куратора {
            request.user.email}: {reason}. Баланс: {old_balance} → {
            balance.amount}",
        performed_by=request.user,
    )
...
def profit_distribution(request):
    ...
    if request.method == "GET":
        return Response(
            {
                ...
                "updated_by": (
                    settings.updated_by.email if settings.updated_by else None
                ),
            }
        )
        ...
        try:
            settings.master_paid_percent = Decimal(
                str(
                    request.data.get(
                        "master_paid_percent", settings.master_paid_percent
                    )
            ...
            settings.master_balance_percent = Decimal(
                str(
                    request.data.get(
                        "master_balance_percent", settings.master_balance_percent
                    )
            ...
            settings.curator_percent = Decimal(
                str(request.data.get("curator_percent", settings.curator_percent))
            )
            settings.company_percent = Decimal(
                str(request.data.get("company_percent", settings.company_percent))
            )
...
def get_balance_with_history(request, user_id):
    ...
    return Response({"current_balance": str(balance), "history": history})
...
def distribute_order_profit(request, order_id):
    ...
    try:
        ...
        master_balance_amount = (
            total_amount * profit_settings["master_balance_percent"] / 100
        )
        ...
        with transaction.atomic():
            ...
            master_balance, _ = Balance.objects.get_or_create(
            ...
            if master_paid_amount > 0 or master_balance_amount > 0:
                FinancialTransaction.objects.create(
                    ...
                    description=f"Прибыль с заказа #{
                        order.id} (выплата: {master_paid_amount}, баланс: {master_balance_amount})",
                )
            ...
            SystemLog.objects.create(
                ...
                description=f"Применены настройки распределения прибыли для заказа #{
                    order.id}",
                performed_by=request.user,
                metadata={
                    "order_id": order.id,
