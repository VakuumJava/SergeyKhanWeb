...
from ..models import ROLES, CustomUser, Order
...
def get_master_workload(request, master_id):
    ...
    assigned_orders = Order.objects.filter(
        assigned_master=master, status__in=["назначен", "выполняется"]
    ).count()
    ...
    workload_data = {
        ...
        "workload_percentage": (
            round((assigned_orders / max_orders_per_day) * 100, 1)
...
def get_best_available_master(request):
    ...
    for master in masters:
        ...
        if current_orders < max_orders:
            workload_percentage = (
                (current_orders / max_orders) * 100 if max_orders > 0 else 0
            )
    ...
    if not available_masters:
        return Response(
            {"error": "No available masters"}, status=status.HTTP_404_NOT_FOUND
        )
...
def assign_order_with_workload_check(request, order_id):
    ...
    if not master_id:
        return Response(
            {"error": "master_id is required"}, status=status.HTTP_400_BAD_REQUEST
        )
