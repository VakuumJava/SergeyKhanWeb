...
def create_order(request):
    ...
    if serializer.is_valid():
        ...
        log_order_action(
            ...
            + (
                f" с планируемым временем {scheduled_date} {scheduled_time}"
            ...
            + (
                f", Планируемое время: {scheduled_date} {scheduled_time}"
                if scheduled_date and scheduled_time
                else ""
            ),
        )
...
def assign_master(request, order_id):
    ...
    if not master_id:
        return Response(
            {"error": "Master ID is required"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    try:
        ...
        return Response(
            {"error": "Invalid master ID"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    if scheduled_date and scheduled_time:
        ...
        # Если дата и время не указаны, проверяем есть ли вообще доступные
        # слоты
        from django.utils import timezone

        available_slots = MasterAvailability.objects.filter(
            master=master, date__gte=timezone.now().date()
        )
    ...
    log_order_action(
        ...
        + (
            f" на {scheduled_date} в {scheduled_time}"
            if scheduled_date and scheduled_time
            else ""
        ),
        ...
        + (
            f", Дата: {scheduled_date}, Время: {scheduled_time}"
            if scheduled_date and scheduled_time
            else ""
        ),
    )
...
def get_assigned_orders(request):
    ...
    # Use OrderDetailSerializer to show full address and details for taken
    # orders
...
def remove_master(request, order_id):
    ...
    if not order.assigned_master:
        return Response(
            {"error": "No master assigned to this order"},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    data = [
        {
            ...
            "original_master": (
                o.assigned_master.email if o.assigned_master else "Не назначен"
            ),
        }
        for o in orders
    ]
