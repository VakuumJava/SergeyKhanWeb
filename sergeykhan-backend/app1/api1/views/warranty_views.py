...
def complete_warranty_order(request, order_id):
    ...
    try:
        ...
        return Response(
            {"error": "Invalid cost/expenses format"},
    ...
    log_order_action(
        ...
        description=f"Заказ #{
            order.id} завершен гарантийным мастером {
            request.user.email}. {completion_notes}",
    ...
    if expenses > 0:
        log_transaction(
            ...
            amount=expenses,
            description=f"Расходы гарантийного мастера по заказу #{
                order.id}: {completion_notes}",
            order=order,
            performed_by=request.user,
        )
...
def approve_warranty_order(request, order_id):
    ...
    try:
        ...
        return Response(
            {"error": "Order not found or not completed by warranty master"},
            status=status.HTTP_404_NOT_FOUND,
        )
    ...
    log_order_action(
        ...
        performed_by=request.user,
        description=f"Заказ #{
            order.id} одобрен администратором {
            request.user.email}",
    ...
    if order.expenses > 0:
        ...
        if master:
            ...
            log_transaction(
                ...
                amount=order.expenses,
                description=f"Выплата гарантийному мастеру за заказ #{
                    order.id}. Баланс: {old_balance} → {
                    balance.amount}",
                order=order,
                performed_by=request.user,
            )
...
def get_warranty_master_stats(request, master_id=None):
    ...
    if master_id:
        try:
            ...
            return Response(
                {"error": "Warranty master not found"}, status=status.HTTP_404_NOT_FOUND
            )
    else:
        master = request.user
    ...
    completed_orders = Order.objects.filter(
        transferred_to=master, status__in=["завершен гарантийный", "одобрен"]
    ).count()
    pending_orders = Order.objects.filter(
        transferred_to=master, status="передан на гарантию"
    ).count()

    # Финансовая статистика
    total_earnings = TransactionLog.objects.filter(
        user=master, transaction_type="master_payment"
...
def complete_transferred_order(request, order_id):
    ...
    try:
        ...
        log_order_action(
            ...
            performed_by=request.user,
            description=f"Переданный заказ #{
                order.id} завершен гарантийным мастером",
