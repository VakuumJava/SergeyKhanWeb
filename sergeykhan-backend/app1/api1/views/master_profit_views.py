...
from ..models import (CustomUser, MasterProfitSettings,
    ...
    try:
        ...
        with transaction.atomic():
            ...
            if not created:
                ...
                settings.master_paid_percent = data.get(
                    "master_paid_percent", settings.master_paid_percent
                )
                settings.master_balance_percent = data.get(
                    "master_balance_percent", settings.master_balance_percent
                )
                settings.curator_percent = data.get(
                    "curator_percent", settings.curator_percent
                )
                settings.company_percent = data.get(
...
def delete_master_profit_settings(request, master_id):
    ...
    try:
        ...
        if settings:
            ...
            return Response(
                {
                    "message": "Индивидуальные настройки удалены. Мастер будет использовать глобальные настройки."
                }
            )
        else:
...
def get_all_masters_with_settings(request):
    ...
    try:
        masters = CustomUser.objects.filter(role="master").order_by(
        ...
        return Response({"masters": masters_data, "total_count": len(masters_data)})
...
def get_order_profit_preview(request, order_id):
    ...
    try:
        ...
        # Проверяем права доступа
        if (
            request.user.role not in ["super-admin"]
            and request.user.id != order.assigned_master_id
        ):
            return Response(
                {"error": "У вас нет прав для просмотра этой информации"},
                status=status.HTTP_403_FORBIDDEN,
        ...
        return Response(
            {
                ...
                "assigned_master": {
                    ...
                    "name": (
                        order.assigned_master.get_full_name()
                ...
                "estimated_distribution": (
                    {
                        "master_paid": (
                            str(
                                (order.final_cost or 0)
                                * profit_settings["master_paid_percent"]
                                / 100
                            )
                            if order.final_cost
                            else None
                        ...
                        "master_balance": (
                            str(
                                (order.final_cost or 0)
                                * profit_settings["master_balance_percent"]
                                / 100
                            )
                            if order.final_cost
                            else None
                        ...
                        "curator_balance": (
                            str(
                                (order.final_cost or 0)
                                * profit_settings["curator_percent"]
                                / 100
                            )
                            if order.final_cost
                            else None
                        ...
                        "company_cash": (
                            str(
                                (order.final_cost or 0)
                                * profit_settings["company_percent"]
                                / 100
                            )
                            if order.final_cost
                            else None
