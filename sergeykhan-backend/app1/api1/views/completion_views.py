...
def complete_order(request, order_id):
    ...
    try:
        ...
        # Проверяем, что заказ назначен текущему мастеру (только для роли
        # мастера)
        if (
            request.user.role == ROLES["MASTER"]
            and order.assigned_master != request.user
        ):
            return Response(
                {"error": "Заказ не назначен вам"}, status=status.HTTP_403_FORBIDDEN
            )
        ...
        serializer = OrderCompletionCreateSerializer(
        ...
        if serializer.is_valid():
            ...
            return Response(
                OrderCompletionSerializer(completion).data,
...
def get_pending_completions(request):
    ...
    completions = OrderCompletion.objects.filter(status="ожидает_проверки").order_by(
        "-created_at"
    )
...
def review_completion(request, completion_id):
    ...
    try:
        ...
        serializer = OrderCompletionReviewSerializer(
            completion, data=request.data, context={"request": request}
        )
        ...
        if serializer.is_valid():
            try:
                ...
                log_order_action(
                    ...
                    action=action_type,
                    performed_by=request.user,
                    description=f"Завершение заказа {
                        completion.status} куратором",
                ...
                if completion.status == "одобрен":
                    try:
                        distribution = distribute_completion_funds(
                            completion, request.user
                        )
                        if distribution:
                            result_data.update(
                                {
                                    "master_payment": distribution.get(
                                        "master_amount", 0
                                    ),
                                    "curator_payment": distribution.get(
                                        "curator_amount", 0
                                    ),
                                    "company_payment": distribution.get(
                                        "company_amount", 0
                                    ),
                                }
                            )
                    except Exception as dist_error:
                        # Если ошибка в распределении средств, возвращаем
                        # успешный ответ но с предупреждением
                        result_data["distribution_error"] = (
                            f"Ошибка распределения средств: {
                            str(dist_error)}"
                        )
        ...
        return Response(
            {"error": "Запись о завершении не найдена"},
...
def get_completion_distribution(request, completion_id):
    ...
    try:
        ...
        if distribution:
            ...
            return Response(
                {"error": "Распределение недоступно"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
def distribute_completion_funds(completion, curator):
    ...
    try:
        ...
        log_order_action(
            ...
            description=f'Начало распределения средств: {
                distribution["settings_used"]} для заказа #{
                completion.order.id}',
        )
        ...
        FinancialTransaction.objects.create(
            ...
            description=f'Выплата за завершение заказа #{
                completion.order.id} ({
                distribution["settings_used"]["cash_percent"]}%)',
        )
        ...
        FinancialTransaction.objects.create(
            ...
            description=f'Отложенная выплата за заказ #{
                completion.order.id} ({
                distribution["settings_used"]["balance_percent"]}%)',
        )
        ...
        FinancialTransaction.objects.create(
            ...
            description=f'Выплата куратору за одобрение заказа #{
                completion.order.id} ({
                distribution["settings_used"]["curator_percent"]}%)',
        )
        ...
        BalanceLog.objects.create(
            ...
            action_type="top_up",  # Используем правильный action_type
            amount=curator_share,
            reason=f'Выплата за проверку заказа #{
                completion.order.id} ({
                distribution["settings_used"]["curator_percent"]}%)',
            performed_by=curator,
        ...
        if company_balance:
            ...
            CompanyBalanceLog.objects.create(
                ...
                reason=f'Доход от завершения заказа #{
                    completion.order.id} ({
                    distribution["settings_used"]["final_kassa_percent"]}%)',
                performed_by=curator,
        ...
        FinancialTransaction.objects.create(
            ...
            amount=company_share,
            description=f'Доход компании от заказа #{
                completion.order.id} ({
                distribution["settings_used"]["final_kassa_percent"]}%)',
        )
...
def get_master_completions(request):
    ...
    completions = OrderCompletion.objects.filter(master=request.user).order_by(
        "-created_at"
    )
    serializer = OrderCompletionSerializer(completions, many=True)
    return Response(serializer.data)
